...
def classify_channel_quality(raw_data: pd.Series, corrected_data: pd.Series,
    ...
    # Detect if this is likely a variable box (small signal range)
    is_variable_box = signal_range < 1.0  # Variable box typically < 1Ω range
    
    # Relative noise level (adjusted for variable box)
    if is_variable_box:
        # Variable box: more permissive noise threshold
        noise_to_signal_ratio = baseline_std / (corrected_range + 1e-10)
    else:
        # Taurus box: stricter threshold
    ...
    elif not baseline_centered and noise_to_signal_ratio > 0.4:
        quality = "MARGINAL"
        usability = "CAUTION"
    ...
    elif baseline_centered and noise_to_signal_ratio < 0.35:
        quality = "GOOD"
        usability = "INCLUDE"
        reason = "Acceptable baseline and noise levels"
        
    elif noise_to_signal_ratio < 0.5:
        quality = "GOOD"
        usability = "INCLUDE"
        reason = "Acceptable noise level for sensor data"
...
if __name__ == "__main__":
    ...
    # Import your baseline correction and analysis functions
